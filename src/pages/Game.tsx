...
import { useWindowSize } from 'react-use';interface Question {
  id: string;
  text: string;
  ...
  const { width, height } = useWindowSize();  useEffect(() => {
    if (!walletAddress || credits < 1) {
      toast.error('You need at least 1 credit to play!');
      navigate('/');
      return;
    }loadQuestions();  }, []);  // Global 30-second timer
  useEffect(() => {
    if (gameState !== 'playing' || !gameStartTime) return;const interval = setInterval(() => {
...
return () => clearInterval(interval);  }, [gameState, gameStartTime]);  const handleGlobalTimeout = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .limit(100);  if (error) throw error;
  ...
  navigate('/');
    ...
    if (!walletAddress) return;const { data: user } = await supabase
  .from('users')
...
if (user) {
  ...
  await refreshBalance();
    ...
    setUserAnswers(newAnswers);if (currentQuestion + 1 < questions.length && globalTimeLeft > 0) {
  ...
  await endGame(finalCorrect);
    ...
    try {
      ...
      const questionIds = questions.map(q => q.id);  const { data, error } = await supabase
    .from('questions')
  ...
  return 0;
    ...
    if (!walletAddress) return;const won = finalCorrect === 3;
...
await loadQuestions();  };  const formatTime = (seconds: number) => {
    ...
    return (
      <div className="min-h-screen bg-gradient-dark flex items-center justify-center p-4 overflow-x-hidden">
        {showConfetti && <Confetti width={width} height={height} />}    <div className="max-w-md w-full space-y-4 md:space-y-6 text-center">
